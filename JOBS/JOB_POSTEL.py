#!/usr/bin/env python
'''
Postel projects the files located in the directory specified by the first
argument around the heliographic coordinates longitude and latitude, specified
by the second and third arguments. The fourth argument is the name of the
projected datacube.
'''


def _postel_worker(args):
    '''
    ...
    from sunpython.map_projection import los2postel
    import os
...
def postel(directory, crln, crlt, name_output, algorithm, doppler=False,
    ...
    if n_workers == 1:
        ...
        with ProcessPoolExecutor(max_workers=n_workers) as executor:
            futures = [executor.submit(_postel_worker, t) for t in tasks]
...
if __name__ == '__main__':
    ...
    def parse_args():
        parser = argparse.ArgumentParser(
            prog='JOB_POSTEL.py',
            description='Project data into a Postel coordinate system.'
        )
