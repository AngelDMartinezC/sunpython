...
'''
    ...
    Usage: lonlat2xy.py REF_FILE[FITS] CRLN CRLT

    Returns: (x, y)
'''


def lonlat2xy(map_input, crln, crlt):
    '''
    Converts from Heliographic Carrington to pixels
    ...
    wcs = map_input.world_to_pixel(c)
    xpix = wcs.x
    ypix = wcs.y
...
if __name__ == '__main__':
    ...
    def parse_args():
        parser = argparse.ArgumentParser(
            description=('Convert Carrington longitude/latitude to pixel '
                         'coordinates.')
        )
    ...
    xpix, ypix = lonlat2xy(map_input, args.crln, args.crlt)
